...
class Auth {
  ...
  async handle ({ request, response, auth }, next) {
    try{
      ...
      if(!refresh_token)return response.status(400).json({message:"missing refresh token"})

      const refreshed_token  = await auth.newRefreshToken().generateForRefreshToken(refresh_token)
